...
/** Convert a px value to Tailwind spacing scale (falls back to arbitrary). */
function spacing(v) {
    const quarters = v / 4;
    if (Number.isInteger(quarters))
        return `${quarters}`.replace('.', '_');
    return `[${v}px]`;
}
...
/** Convert one CSS declaration to Tailwind classes. Returns [] if unknown. */
function pick(map, v) {
    const m = map[v];
    return m ? [m] : [];
}
function toTailwind(prop, value) {
    const v = value.trim();
    const p = px(v);
    switch (prop) {
        ...
        case 'padding': {
            ...
            if (parts.every((n) => n !== null)) {
                const [t, r, b, l] = parts;
                if (parts.every((n) => n === parts[0]))
                    return [`${prefix}-${spacing(parts[0])}`];
                if (t === b && r === l)
                    return [`${prefix}-y-${spacing(t)}`, `${prefix}-x-${spacing(r)}`];
                if (r === l)
                    return [`${prefix}-t-${spacing(t)}`, `${prefix}-x-${spacing(r)}`, `${prefix}-b-${spacing(b)}`];
...
/** Convert a node's styles to Tailwind utility classes. */
export function tailwindClasses(node) {
    const classes = [];
    for (const [prop, value] of Object.entries(node.styles)) {
        for (const c of toTailwind(prop, value)) {
            if (typeof c === 'string' && c && !classes.includes(c))
                classes.push(c);
        }
    }
    return classes;
