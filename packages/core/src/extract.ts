...
/**
 * Page-side extraction function. Serialized and evaluated inside the browser:
 * walks the subtree, prunes default styles, serializes to IR.
...
function __snapuiExtract(rootEl, opts) {
  ...
  // Cache of default computed styles per tag (from a clean clone).
  // The probe MUST be attached to the document: getComputedStyle on a
  // detached element returns empty strings in Chrome, which silently
  // disabled every default-value prune.
  ...
  function cleanStyles(el, cs, parentCS) {
    ...
    for (const p of PROPS) {
      ...
      // Context-dependent boxes are meaningless outside their display mode.
  ...
  function walk(el, depth, parentCS) {
    ...
    // Text-only leaf: inline text content of elements without element children.
    ...
    // SVG children: keep raw outerHTML as special node (generators inline it).
    // Strip framework-junk attributes (data-astro-cid, data-reactid, …).
