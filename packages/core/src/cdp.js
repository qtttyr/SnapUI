...
export function chromeFolderName(platformTag) {
    const map = {
        mac_arm: 'mac-arm64',
        mac: 'mac-x64',
        linux: 'linux64',
    ...
    if (!folder)
        throw new Error(`SnapUI requires Google Chrome: unsupported platform "${platformTag}".`);
    return folder;
}
/** Direct download URL for the Chrome-for-Testing archive. */
export function chromeDownloadUrl(platformTag, buildId) {
...
function chromeExecutableRel(platformTag) {
    const folder = chromeFolderName(platformTag);
    ...
    try {
        ...
        if (platform() === 'darwin') {
            ...
            for (const fw of readdirSync(frameworksDir)) {
                ...
                if (existsSync(bin) && statSync(bin).size > 30 * 1024 * 1024)
                    return true;
            }
            return false;
        }
...
function unpackChromeZip(zipPath, destDir) {
    const p = platform();
    if (p === 'darwin') {
...
export function findChromeExecutable() {
    ...
    if (p === 'darwin') {
        ...
        paths.push(join(localAppData, 'Google', 'Chrome', 'Application', 'chrome.exe'), 'C:\\Program Files\\Google\\Chrome\\Application\\chrome.exe', 'C:\\Program Files (x86)\\Google\\Chrome\\Application\\chrome.exe');
    }
    else {
        paths.push('/usr/bin/google-chrome', '/usr/bin/google-chrome-stable', '/opt/google/chrome/chrome');
    }
 ...
 */
export async function attachToRunningChrome(port = 9222) {
 ...
 */
export async function launchIsolatedChrome(url, opts) {
    let executablePath = process.env.SNAPUI_FORCE_MANAGED === '1' ? null : findChromeExecutable();
    if (!executablePath) {
...
export async function ensureManagedBrowser() {
    ...
    if (!platformTag) {
        throw new Error('SnapUI requires Google Chrome. This platform cannot run the managed Chrome download — please install Google Chrome (https://www.google.com/chrome) and re-run SnapUI.');
    }
    ...
    rmSync(dir, { recursive: true, force: true });
    throw new Error('SnapUI requires Google Chrome but could not install its private copy automatically.\n' +
...
export async function ensurePageAt(session, url) {
    if (!url)
        return;
...
export function uninstallSnapUI() {
