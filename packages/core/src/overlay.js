/**
 * Browser-side overlay for element picking.
 * Injected via page.evaluate; exposes window.__snapuiPick which resolves
 * with the picked Element (or null on Esc).
 */
export const OVERLAY_SOURCE = /* js */ `
(() => {
  if (window.__snapuiPick) return; // already active
  ...
  function onClick(e) {
    const el = document.elementFromPoint(e.clientX, e.clientY);
    if (isSnapUIElement(el)) return; // Don't block panel interaction!
  ...
  function finish(el) {
    ...
    state.done = true;
    cleanup();
    // Keep the element IN-PAGE: DOM handles must not cross the CDP boundary
    // (newer Chrome versions serialize nodes as plain {} — extraction runs in-page).
