/**
 * In-browser control panel: a right-side drawer injected into the captured
 * page. Mirrors the whole CLI flow (pick → configure → generated code) so the
 * user never leaves the browser. Communication: Node pushes state via
 * window.__snapuiPanelState(state); the panel posts user actions through the
 * exposed __snapuiPanelAction(action) binding.
 */
export interface PanelState {
  phase: 'picking' | 'configuring' | 'generating' | 'generated' | 'error' | 'cancelled';
  hint?: { tag: string; classes: string[]; id?: string; textSample: string; nodes?: number } | null;
  suggestion?: string;
  framework?: string;
  style?: string;
  error?: string | null;
