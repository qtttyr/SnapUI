 ...
 */
export function assignClassNames(root, componentName) {
    const used = new Set();
    ...
    function walk(node, parentClass) {
        const raw = node.attrs.class?.trim();
...
export function classedNodes(root) {
    const out = [];
    const stack = [root];
    while (stack.length) {
        const n = stack.pop();
        if (n.attrs['data-snap-class'] && n.attrs['data-snap-class'] !== '__skip__')
            out.push(n);
